...
class MicrogridConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if self.battery.max_discharge_power_kw < self.load_profile.peak_load:
            warnings.append(
                f"ℹ️ EXPECTED: Battery power ({self.battery.max_discharge_power_kw} kW) "
        ...
        if self.battery.usable_capacity_kwh < required_full_backup:
            warnings.append(
                f"ℹ️ EXPECTED: Battery capacity ({self.battery.usable_capacity_kwh} kWh) "
        ...
        if self.generator.rated_power_kw < self.load_profile.peak_load:
            warnings.append(
                f"ℹ️ EXPECTED: Generator ({self.generator.rated_power_kw} kW) "
        ...
        if self.pv.estimated_daily_generation_kwh >= daily_consumption * 0.9:
            warnings.append(
                f"⚠️ UNREALISTIC: PV generation too high. "
        ...
        if max_gen_hours > 12:
            warnings.append(
                f"ℹ️ Fuel tank allows {max_gen_hours:.1f}h generator runtime at full load"
            )
