...
class Battery:
    """Battery Energy Storage System - Hospital-grade"""
...
class PVArray:
    """Solar PV array - Hospital rooftop system"""
...
class Generator:
    """Hospital diesel generator with fast startup"""
    
    def __init__(self, config, gen_id: int = 1):
        self.config = config
        self.gen_id = gen_id  # 1 or 2 for dual generator system
    ...
    def update(self, dt_seconds: float, load_demand_kw: float = None) -> float:
        ...
        if self.state == ComponentState.STARTING:
            ...
            if load_demand_kw is not None:
                # Use gen1 or gen2 rated power
                rated_power = self.config.gen1_rated_power_kw if self.gen_id == 1 else self.config.gen2_rated_power_kw
                ...
                # Ramp rate limit
                max_ramp = rated_power * 0.1 * dt_seconds
            ...
            efficiency_factor = 0.7 + 0.3 * load_factor
...
class Load:
    """Hospital load model with critical/non-critical separation"""
    
    def __init__(self, config):
        self.config = config
        
        # Core loads (FIXED: 320 kW critical, 280 kW non-critical)
        self.total_load_kw = 0
        self.critical_load_kw = config.load_profile.total_critical_load  # 320 kW
        self.flexible_load_kw = 0
        self.shed_load_kw = 0
        
        # Non-critical tier loads (hospital-specific)
        self.tier_loads = {
            "HVAC": config.load_profile.hvac_partial_kw,      # 120 kW
            "WARDS_LIGHTING": config.load_profile.wards_lighting_kw,  # 60 kW
            "ADMIN": config.load_profile.admin_misc_kw        # 100 kW
        }
    ...
    def restore_load(self, amount_kw: float = None) -> float:
        ...
        self.flexible_load_kw += actual_restore
    ...
    def shed_tier(self, tier: str, amount_kw: float) -> float:
        """Shed load from specific tier (hospital load shedding)"""
    ...
    def restore_tier(self, tier: str, amount_kw: float) -> float:
        ...
        self.flexible_load_kw += restored
