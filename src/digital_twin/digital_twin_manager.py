...
from src.digital_twin.outage_event_model import ScenarioConfig, OutageType, OutageEvent
from src.digital_twin.scenario_engine import ScenarioEngine

# Import enhanced modules (place these in DigitalTwin folder)
from src.digital_twin.shadow_simulator import ShadowSimulator, PredictionScenario, PredictionResult
from src.digital_twin.state_estimator import CityStateEstimator, StateEstimate
from src.digital_twin.resilience_metrics import EnhancedResilienceMetricCalculator, DetailedResilienceScorecard

# Import Simulators and EMS
from src.microgrid.Hospital.hospital_simulator import MicrogridSimulator as HospitalSim
from src.microgrid.Hospital.hospital_parameters import create_default_config as create_hospital_config

from src.microgrid.university_microgrid.university_simulator import MicrogridSimulator as UniversitySim
from src.microgrid.university_microgrid.Uni_parameters import create_default_config as create_university_config

from src.microgrid.Industry_microgrid.industrial_simulator import MicrogridSimulator as IndustrialSim
from src.microgrid.Industry_microgrid.industrial_parameters import create_default_config as create_industrial_config

from src.microgrid.residence.residential_simulator import MicrogridSimulator as ResidentialSim
from src.microgrid.residence.residential_parameters import create_default_config as create_residential_config

from src.ems.city_ems import CityEMS, CityWideMeasurements, MicrogridStatus, MicrogridInfo, MicrogridPriority, CityOperationMode

# Import Coordinated Outage components
try:
    from src.ems.coordinated_outage import (
        CascadeController, OutageMetricsCollector, RecoveryPhase
    )
...
class EnhancedDigitalTwinManager:
    ...
    def __init__(self, enable_shadow_simulation: bool = True, enable_state_estimation: bool = True):
        ...
        # 7. Coordinated Outage components
        if OUTAGE_COORD_AVAILABLE:
            self.cascade_controller = CascadeController()
            self.outage_metrics = OutageMetricsCollector()
            logger.info("Coordinated Outage components initialized")
        else:
            self.cascade_controller = None
            self.outage_metrics = None
    ...
    def _register_microgrid(self, mg_id: str, config, priority: MicrogridPriority, mg_type: str):
        ...
        info = MicrogridInfo(
            ...
            can_share_power=True  # Enable inter-microgrid resource sharing
        )
    ...
    def run_enhanced_simulation(self, scenario_config: ScenarioConfig, use_predictive_control: bool = True) -> Dict:
        ...
        for step in range(steps):
            ...
            # =====================================
            # Cascade Check + Outage Metrics
            # =====================================
            if self.cascade_controller and self.scenario_engine:
                faulted = self.scenario_engine.get_faulted_microgrids(current_time)
                cascades = self.cascade_controller.check_cascade(
                    mg_statuses, self.city_ems.microgrids,
                    faulted, current_time
                )
                for cascade_event in cascades:
                    self.scenario_engine.inject_event(cascade_event)
            
            if self.outage_metrics:
                recovery_phase = (
                    self.city_ems.recovery_controller.phase
                    if self.city_ems.recovery_controller
                    else RecoveryPhase.INACTIVE
                )
                self.outage_metrics.update(
                    mg_statuses,
                    resource_transfers=(
                        last_city_output.resource_transfers
                        if last_city_output else []
                    ),
                    recovery_phase=recovery_phase,
                    gen_shared_kwh=(
                        self.city_ems.gen_scheduler.total_shared_kwh * 0.25
                        if self.city_ems.gen_scheduler else 0
                    ),
                    demand_reduced_kwh=(
                        self.city_ems.demand_propagator.total_reduction_kwh
                        if self.city_ems.demand_propagator else 0
                    ),
                )
            ...
            # =====================================
            # Track Resource Sharing Transfers
            # =====================================
            if last_city_output and last_city_output.resource_transfers:
                for t in last_city_output.resource_transfers:
                    logger.debug(
                        f"Transfer: {t.from_mg}→{t.to_mg} "
                        f"{t.power_kw:.1f}kW (delivered {t.delivered_kw:.1f}kW)"
                    )
            
            # Progress logging
            if step % 20 == 0:
                sharing_info = ""
                if (self.city_ems.exchange_bus and 
                    self.city_ems.exchange_bus.metrics.total_transfers > 0):
                    sm = self.city_ems.exchange_bus.metrics
                    sharing_info = (
                        f", Sharing: {sm.total_energy_exchanged_kwh:.1f}kWh "
                        f"({sm.total_transfers} transfers)"
                    )
                logger.info(
                    ...
                    f"Confidence={confidence:.3f}{sharing_info}"
                )
        ...
        return {
            ...
            "resource_sharing": (
                self.city_ems.exchange_bus.get_metrics()
                if self.city_ems.exchange_bus else {}
            ),
            "coordinated_outage": (
                self.outage_metrics.finalize(
                    cascade_controller=self.cascade_controller,
                    stability_monitor=self.city_ems.stability_monitor,
                    recovery_controller=self.city_ems.recovery_controller,
                    exchange_bus=self.city_ems.exchange_bus,
                ) if self.outage_metrics else {}
            ),
        }
