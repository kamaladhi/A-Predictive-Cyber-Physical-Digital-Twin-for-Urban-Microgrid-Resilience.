...
class MicrogridFactory:
    ...
    def load_config(microgrid_type: str):
        ...
        if microgrid_type == MicrogridType.HOSPITAL:
            ...
            from src.microgrid.residence.residential_parameters import create_default_config
            return create_default_config()
            
        elif microgrid_type == MicrogridType.INDUSTRIAL:
            from src.microgrid.Industry_microgrid.industrial_parameters import create_default_config
            return create_default_config()
    ...
    def load_simulator(microgrid_type: str, config):
        ...
        if microgrid_type == MicrogridType.HOSPITAL:
            ...
            from src.microgrid.university_microgrid.university_simulator import MicrogridSimulator
            return MicrogridSimulator(config)
            
        elif microgrid_type == MicrogridType.RESIDENCE:
            from src.microgrid.residence.residential_simulator import MicrogridSimulator
            return MicrogridSimulator(config)
            
        elif microgrid_type == MicrogridType.INDUSTRIAL:
            from src.microgrid.Industry_microgrid.industrial_simulator import MicrogridSimulator
            return MicrogridSimulator(config)
